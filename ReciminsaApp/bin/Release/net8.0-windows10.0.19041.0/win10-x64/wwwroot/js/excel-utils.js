...
function exportarExcelResiduos(invoice) {
    ...
    try {
        ...
        XLSX.writeFile(wb, `Residuos_${invoice.id}_${(invoice.date || '').replace(/-/g, '')}.xlsx`);
...
function exportSelectedDataToExcel(selection = {}) {
    ...
    try {
        ...
        XLSX.writeFile(wb, `Reciminsa_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
...
function exportBitacorasListToExcel(bitacoras) {
    ...
    try {
        ...
        XLSX.writeFile(wb, `Bitacora_Recogida_${new Date().toISOString().split('T')[0]}.xlsx`);
