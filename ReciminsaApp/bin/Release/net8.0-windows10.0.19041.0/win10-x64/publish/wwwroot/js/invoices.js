...
function saveInvoice(invoice) {
  ...
  // --- INTEGRACION MAUI OFFLINE (SQLITE) ---
  if (window.DotNet) {
    window.DotNet.invokeMethodAsync('ReciminsaApp', 'SaveOfflineRecord', 'invoices', JSON.stringify(invoice))
      .then(id => console.log('Guardado en SQLite Offline con ID:', id))
      .catch(err => console.warn('No se pudo guardar en SQLite MAUI:', err));
  }
}
